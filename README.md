...
本目录是「千代福网站改版 / 改版第五版」中 `index_微调预览V3.3.html` 的**可直接发布静态站**。

## 目录结构
```
发布_千代福预览站/
├── index.html                      # 根入口（自动跳转到预览页）
├── index_微调预览V3.3.html         # 预览页本体（按你的要求保留原名）
...
- 文件名含中文，`index_微调预览V3.3.html` 在 URL 中会被编码为
  `index_%E5%BE%AE%E8%B0%83%E9%A2%84%E8%A7%88V3.3.html`，因此根目录用 `index.html` 做跳转。
